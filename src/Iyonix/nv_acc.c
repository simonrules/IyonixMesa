...
//outdated (pre-NV04):
	...
	si->engine.fifo.handle[1] = NV_IMAGE_BLACK_RECTANGLE;
	si->engine.fifo.handle[2] = NV_IMAGE_PATTERN;
	si->engine.fifo.handle[3] = NV_IMAGE_BLIT;
	si->engine.fifo.handle[4] = NV3_GDI_RECTANGLE_TEXT;
	si->engine.fifo.handle[5] = NV1_RENDER_SOLID_LIN;
	si->engine.fifo.handle[6] = NV10_CONTEXT_SURFACES_2D;
	si->engine.fifo.handle[7] = NV4_DX5_TEXTURE_TRIANGLE;
...
void nv_acc_assert_fifo(void)
{
	...
	if (!si->engine.fifo.ch_ptr[NV_ROP5_SOLID] ||
		...
		!si->engine.fifo.ch_ptr[NV1_RENDER_SOLID_LIN] ||
		!si->engine.fifo.ch_ptr[NV10_CONTEXT_SURFACES_2D] ||
