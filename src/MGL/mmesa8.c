...
void _mmesa_write_span_ci8(GLcontext *ctx,GLuint n,GLint x,GLint y,
	GLubyte index[],GLubyte mask[])
{
	uchar *d = PACKED8_pixelAddr(x,FLIP(y));
	while (n >= 8) {
		if (mask[0]) d[0] = (uchar)index[0];
...
IMPLEMENT_WRITE_SPAN_RGB(8,8,uchar);
IMPLEMENT_WRITE_SPAN_RGB_DITHER(8,DITHER8,uchar);
...
void _mmesa_read_span_8_8(GLcontext *ctx,GLuint n,GLint x,GLint y,
	GLubyte rgba[][4])
{
	...
	while (n--) {
		color = colorTab[d[0]];
		UNPACK_COLOR_RGB(color,rgba[0][RCOMP],rgba[0][GCOMP],rgba[0][BCOMP]);
		rgba++; d++;
		}
}
...
void _mmesa_read_pixels_8_8(GLcontext *ctx,GLuint n,GLint x[],GLint y[],
